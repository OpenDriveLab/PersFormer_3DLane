...
class PersFormer(nn.Module):
    def __init__(self, args):
        ...
        self.bev_head = BEVHead(args, channels=128)
        # 3d lane detector
        self.lane_out = Lane3D.LanePredictionHead(args.feature_channels * pow(2, self.num_proj - 2),
                                                  ...
                                                  args.no_cuda)
        # segmentation head
        self.segment_head = SegmentHead(channels=128)
    ...
    def forward(self, input, _M_inv = None, eval=False, use_att=True, args=None):
...
class PerspectiveTransformer(nn.Module):
    def __init__(self, args, channels, bev_h, bev_w, uv_h, uv_w, M_inv, num_encode_layer, num_proj, nhead, npoints):
        ...
        for i in range(self.num_proj):
            ...
            for j in range(self.num_encode_layer):
    ...
    def forward(self, input, frontview_features, _M_inv = None):
        ...
        for i in range(self.num_proj):
            ...
            for j in range(self.num_encode_layer):
                query_embed = self.el[i*self.num_encode_layer+j](query=query_embed, value=src, bev_pos=bev_pos, 
